class Player extends Sprite{
    ...
    checkForHorizontalCanvasCollision() {
      if (this.hitbox.position.x + this.hitbox.width + this.velocity.x + 27 >= 576 ||
    ...
    shouldPanCameraToTheRight({canvas, camera}) {
      ...
      const scaledDownCanvas = canvas.width / 4

      if (cameraboxRightSide >= 576) return

      if (cameraboxRightSide >= scaledDownCanvas + Math.abs(camera.position.x)) {
